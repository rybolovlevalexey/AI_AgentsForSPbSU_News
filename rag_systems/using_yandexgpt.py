...
# YandexEmbeddings + YandexGPT - не работает из-за yandexcloud.
dotenv.load_dotenv()
...
def request_func_with_context_by_YandexEmbeddings():
    loader = DirectoryLoader(
        r"C:\Users\rybol\PycharmProjects\AIAgentsForSPbSU_News\university_data")
    documents = loader.load()
    text_splitter = RecursiveCharacterTextSplitter(
    ...
    embeddings = YandexGPTEmbeddings(api_key=secret_key, folder_id=catalog_id)
    db = Chroma.from_documents(
        documents,
        embeddings,
        client_settings=Settings(anonymized_telemetry=False),
